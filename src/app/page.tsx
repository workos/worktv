...
import {
  ...
  searchRecordingsWithParticipant,
...
import { SourceFilter } from "./recordings/source-filter";
import { ClipsListView } from "./recordings/clips-list-view";
import { LocalDateTime } from "@/components/local-datetime";
import { RecordingPreview } from "./recordings/recording-preview";
import { RecordingGridCard } from "./recordings/recording-grid-card";
import { SearchMatchDisplay } from "./recordings/search-match-display";
import { RecordingsInfiniteScroll } from "./recordings/recordings-infinite-scroll";
import { SearchResultsWrapper } from "./recordings/search-results-wrapper";
...
export default async function HomePage({
  ...
  // Use full search for queries/speaker/participant filters, paginated for browsing
  const isSearchMode = Boolean(q || speakers.length > 0 || participant);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {(q || speakers.length > 0 || participant) && (
        <div className="flex items-center gap-2 text-sm text-zinc-400 light:text-zinc-600">
          <span>
            ...
            {participant && (
              <>
                {" "}with <span className="font-medium text-emerald-400 light:text-emerald-600">{participant}</span>
              </>
            )}
            {q && (
              <>
